<?php

return [
    'login' => [
        'banned' => 'Esta cuenta está suspendida.',
        'dont_have_account' => 'No tiene una cuenta en este sistema, comuníquese con el administrador para obtener más información.',
        'email' => 'Correo electrónico',
        'fail' => 'Nombre de usuario o contraseña incorrectos.',
        'login' => 'ACCESO',
        'logout_success' => 'Sesión Cerrada!',
        'not_active' => '¡Tu cuenta no ha sido activada todavía!',
        'password' => 'Contraseña',
        'placeholder' => [
            'email' => 'Por favor introduzca su correo electrónico',
            'username' => 'Por favor, ingrese su nombre de usuario',
            'password' => 'Ingresa tu contraseña',
        ],
        'remember' => '¿Recuérdame?',
        'success' => 'Inició sesión correctamente!',
        'title' => 'Inicio de sesión de usuario',
        'username' => 'Correo electrónico / Nombre de usuario',
    ],
    'forgot_password' => [
        'message' => '<p> ¿Ha olvidado su contraseña? </p> <p> Ingrese su cuenta de correo electrónico. El sistema enviará un correo electrónico con un enlace activo para restablecer su contraseña. </p>',
        'submit' => 'Enviar',
        'title' => 'Has olvidado tu contraseña?',
    ],
    'reset' => [
        'email' => 'Correo electrónico',
        'fail' => 'El token no es válido, el enlace para restablecer la contraseña ha caducado.',
        'new_password' => 'Nueva contraseña',
        'new-password' => 'Nueva contraseña',
        'password_confirmation' => 'Confirmar nueva contraseña',
        'reset' => [
            'title' => 'Correo electrónico restablecer contraseña',
        ],
        'send' => [
            'fail' => 'No se puede enviar correo electrónico en este momento. Por favor, inténtelo de nuevo más tarde.',
            'success' => 'Se envió un correo electrónico a su cuenta de correo electrónico. Verifique y complete esta acción.',
        ],
        'success' => 'Se restableció la contraseña!',
        'update' => 'Actualizar',
        'title' => 'Restablecer su contraseña',
        'user_not_found' => 'Este nombre de usuario no existe.',
        'wrong_token' => 'Este enlace no es válido o ha caducado. Intente usar el formulario de restablecimiento nuevamente.',
        'placeholder' => [
            'new_password' => 'Ingrese su nueva contraseña',
            'new_password_confirmation' => 'Confirme su nueva contraseña',
        ],
    ],
    'email' => [
        'reminder' => [
            'title' => 'Restablecimiento de contraseña por correo electrónico',
        ],
    ],
    'password_confirmation' => 'Confirmación de contraseña',
    'failed' => 'Fallido',
    'throttle' => 'Limitación de velocidad',
    'not_member' => '¿Todavía no eres miembro?',
    'register_now' => 'Regístrate ahora',
    'lost_your_password' => '¿Olvidaste tu contraseña?',
    'login_title' => 'Administrador',
    'login_via_social' => 'Iniciar sesión con redes sociales',
    'back_to_login' => 'Volver a la página de inicio de sesión',
    'sign_in_below' => 'Iniciar sesión',
    'languages' => 'Idiomas',
    'reset_password' => 'Restablecer la contraseña',
    'settings' => [
        'email' => [
            'description' => 'Configuración de correo ACL',
            'title' => 'LCA',
            'templates' => [
                'password_reminder' => [
                    'title' => 'Restablecer contraseña',
                    'description' => 'Enviar correo electrónico al usuario al solicitar restablecimiento de contraseña',
                    'subject' => 'Restablecer contraseña',
                    'reset_link' => 'Enlace para restablecer la contraseña',
                ],
            ],
        ],
    ],
];
