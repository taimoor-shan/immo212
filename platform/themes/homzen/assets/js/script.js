...
$(() => {
    ...
    $(".price-dropdown-select").each(function () {
      ...
      $trigger.on("click", function (e) {
        ...
        $(".price-dropdown-select")
          .not($dropdown)
          .find(".price-dropdown-menu")
          .removeClass("show");
        $(".price-dropdown-select")
          .not($dropdown)
          .find(".price-select-trigger")
          .removeClass("open");
      ...
      const updateDisplayText = function () {
        ...
        if (minVal || maxVal) {
          const minText = minVal
            ? new Intl.NumberFormat().format(minVal)
            : "Any";
          const maxText = maxVal
            ? new Intl.NumberFormat().format(maxVal)
            : "Any";
  ...
  const rangeSlider = () => {
    ...
    const priceSlider = () => {
      ...
      $('[data-bb-toggle="range"]').each((index, el) => {
        ...
        const rangeSlider = $element
          .find('[data-bb-toggle="range-slider"]')
          .get(0);
        const $minInput = $element.find(
          '.slider-labels input[data-bb-toggle="min-input"]',
        );
        const $maxInput = $element.find(
          '.slider-labels input[data-bb-toggle="max-input"]',
        );
        ...
        if ($(rangeSlider).data("currency-prefix-symbol")) {
          moneyFormatOptions.prefix =
            currencySymbol + (currencyWithSpace ? " " : "");
        } else {
          moneyFormatOptions.postfix =
            (currencyWithSpace ? " " : "") + currencySymbol;
        }
        ...
        noUiSlider.create(rangeSlider, {
          start: [
            parseInt($minInput.val() || $element.data("min")) || 0,
            parseInt($maxInput.val() || $element.data("max")) || 0,
          ],
          step: 1,
          range: {
    ...
    const squareSlider = () => {
      ...
      noUiSlider.create(rangeSlider, {
        start: [
          parseInt($minSquare.val() || $(rangeSlider).data("min")),
          parseInt($maxSquare.val() || $(rangeSlider).data("max")),
        ],
        step: 1,
        range: {
      ...
      rangeSlider.noUiSlider.on("change", function (values) {
        $('.slider-labels input[name="min_square"]')
          .val(moneyFormat.from(values[0]))
          .trigger("change");
        $('.slider-labels input[name="max_square"]')
          .val(moneyFormat.from(values[1]))
          .trigger("change");
      });
    };
    ...
    const flatSlider = () => {
      ...
      noUiSlider.create(rangeSlider, {
        start: [
          parseInt($minFlat.val() || $(rangeSlider).data("min")),
          parseInt($maxFlat.val() || $(rangeSlider).data("max")),
        ],
        step: 1,
        range: {
          min: [$(rangeSlider).data("min")],
      ...
      rangeSlider.noUiSlider.on("change", function (values) {
        $('.slider-labels input[name="min_flat"]')
          .val(moneyFormat.from(values[0]))
          .trigger("change");
        $('.slider-labels input[name="max_flat"]')
          .val(moneyFormat.from(values[1]))
          .trigger("change");
      });
    };
  ...
  const initPropertiesTab = () => {
    $(document)
      ...
      .on(
        "click",
        '[data-bb-toggle="properties-tab"] [data-bs-toggle="tab"]',
        (e) => {
          const currentTarget = $(e.currentTarget);
          const tab = currentTarget.closest(
          ...
          $.ajax({
            ...
            success: ({ data }) => {
              parentTab
                .find('[data-bb-toggle="properties-tab-slot"]')
                .html(data);
  ...
  function cleanFormData(formDataInput) {
    const formData = formDataInput.filter(
      (item) =>
        item.value !== "" &&
        (item.name !== "per_page" ||
          (item.name === "per_page" && parseInt(item.value) !== 12)),
    );
    ...
    if ($("#tf-sw-blog-v8").length > 0) {
      ...
      const swiper = new Swiper("#tf-sw-blog-v8", {
        ...
        on: {
          init: function () {
            console.log(
              "Blog carousel initialized with",
              this.slides.length,
              "slides",
            );
          },
          ...
          success: ({ data, meta }) => {
            ...
            data.forEach((item) => {
              ...
              if (isVacationRental) {
                content = content
                  .replace(
                    new RegExp("__minimum_stay__", "gi"),
                    item.minimum_stay || "",
                  )
                  .replace(
                    new RegExp("__maximum_guests__", "gi"),
                    item.maximum_guests || "",
                  )
                  .replace(
                    new RegExp("__category__", "gi"),
                    item.category_name || "",
                  );
              } else if (isProperty) {
                content = content
                  ...
                  .replace(
                    new RegExp("__bathroom__", "gi"),
                    item.number_bathroom,
                  )
  ...
  let projectSearchTimeout = null;
    ...
    wishlistArray.forEach((id) => {
      const $button = $(
        `[data-bb-toggle="add-to-wishlist"][data-type="property"][data-id="${id}"]`,
      );
  ...
  $(document)
    ...
    .on(
      "change",
      '.filter-form select[name="sort_by"], .filter-form select[name="per_page"]',
      (e) => {
    ...
    .on(
      "keyup",
      '[data-bb-toggle="search-suggestion"] input[type="text"]',
      (e) => {
        ...
        projectSearchTimeout = setTimeout(() => {
          $.ajax({
            url:
              ...
              $currentTarget.closest("form").prop("action"),
            type: "GET",
            data: cleanedFormData.formData,
            success: ({ data }) => {
    ...
    .on("click", ".search-suggestion-item:not([data-no-prevent])", (e) => {
      const $currentTarget = $(e.currentTarget);
      const $search = $currentTarget.closest(
    ...
    .on("click", '[data-bb-toggle="add-to-wishlist"]', (e) => {
      ...
      if (wishlistArray.includes(String(id))) {
        ...
        // Update the icon
        const $icon = $currentTarget.find(".icon, svg");
        if ($icon.length) {
          $icon.replaceWith(`
        ...
        const $text = $currentTarget.find("span");
        if ($text.length) {
          $text.text("Save");
        }
  ...
  $(
    `[data-bb-toggle="change-search-type"][data-value="${$(".flat-tab").find('form input[name="type"]')}"]`,
  ).trigger("click");
  ...
  document.addEventListener("shortcode.loaded", (e) => {
    ...
    switch (name) {
      ...
      case "blog-posts":
        console.log(
          "Blog posts shortcode loaded with style:",
          attributes.style,
        );
  ...
  const enhancedLocationDropdown = function () {
    ...
    function enhanceLocationDropdown() {
      ...
      if (!$locationNiceSelect.find(".nice-select-search-wrapper").length) {
        ...
        $list.prepend(
          '<div class="nice-select-search-wrapper"><input type="text" class="nice-select-search" placeholder="Search for a city..."/></div>',
        );

        // Add loading indicator at the bottom of the list
        $list.append(
          '<div class="nice-select-loader" style="display:none;"><div class="spinner"></div></div>',
        );
    ...
    function setupEventHandlers($niceSelect) {
      ...
      $list.on("scroll", function () {
        ...
        if (
          !loadingMore &&
          !allItemsLoaded &&
          scrollPosition + listHeight > scrollHeight - 50
        ) {
          page++;
      ...
      $niceSelect.on("click", function (e) {
        if (
          $niceSelect.hasClass("open") &&
          !$(e.target).hasClass("nice-select-search")
        ) {
        ...
        success: function (response) {
          ...
          if (typeof response === "string") {
            ...
            $tempElement.find(".search-suggestion-item").each(function () {
              ...
              const cityId =
                $(this).data("value") || generateTempId(cityFullName);
    ...
    function updateCityOptions($niceSelect, cities, append) {
      ...
      cities.forEach(function (city) {
        ...
        $loader.before(
          `<li data-value="${city.id}" class="option">${city.text}</li>`,
        );
      });
  ...
  setTimeout(() => {
    if (
      $("#tf-sw-blog-v8").length > 0 &&
      !$("#tf-sw-blog-v8").hasClass("swiper-initialized")
    ) {
